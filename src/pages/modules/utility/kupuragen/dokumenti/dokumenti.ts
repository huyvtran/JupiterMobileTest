...
@IonicPage()
@Component({
  selector: 'page-utility-dokumenti-kupuragen',
  templateUrl: 'dokumenti.html',
  providers: [File, FileOpener, UtilityDMSService]
})
export class UtilityKupUraGenDokumentiPage extends BasePage {
  data: any;
  dokument: any
  settings: any
  ...
  getData() {
    this.getDataDef().then(res => {
      ...
      this.settings = res ? res.settings : null;
  ...
  pregled(item) {
    ...
    if (item.attachments.length === 0)
      ...
      this.navCtrl.push("UtilityKupUraGenDokumentiViewPage", { item })
    }
  ...
  presentModal(data) {
    this.navCtrl.push("UtilityKupUraGenDokumentiViewPage", { data: data })
  ...
  azur(kupprometid: number, status: string, data: any) {
    console.log("azur");
    console.log(data);
    let dataDef: ICore.IData = {
      "queries": [
        {
          ...
          "params": {
            ...
            "memo2": data.memo2,
            "memo3": data.memo3,
            "orgshemaid": data.orgshemaid,
            "nacinnaplateid": data.nacinnaplateid
          }
        }
      ]
    }
  ...
  presentModalDokumentDet(item, status: string) {
    ...
    try {
      this.global.modal = this
        .modalCtrl
        .create('UtilityKupUraGenDokumentDetPage', { item, status: status });
      this.global.modal.present();
      this.global.modal.onDidDismiss(data => {
        console.log(data);
        if (data != null && data.type == "ok") {
          try {
            this.azur(item.kupprometid, status, data.parametri).then(x => {
              this.getData();
            });
          } catch (e) {
