import { Component } from '@angular/core';
import { IonicPage, NavController, NavParams, ModalController } from 'ionic-angular';
import { BasePage } from '../../../../../providers/base/base-page';
import { UtilityIzvidnikProvider } from '../../../../../providers/modules/utility/izvidnik/utility-izvidnik-provider';

/**
 * Generated class for the DanasPage page.
 *
 * See http://ionicframework.com/docs/components/#navigation for more info
 * on Ionic pages and navigation.
 */

@IonicPage()
@Component({
  selector: 'page-danas',
  templateUrl: 'danas.html',
})
export class DanasPage extends BasePage {

  dogadjaji:any = []
  subTitle: string
  filterType: string

  constructor(public navCtrl: NavController, public navParams: NavParams, private izvidnikProvider:UtilityIzvidnikProvider, private modalCtrl: ModalController) {
    super();

    this.filterType=navParams.data;

    if (this.filterType=="mojidogadjaji") {
        this.subTitle = "Moji događaji";

    }   
    else if (this.filterType=="danas") {
        this.subTitle = "Današnji događaji";

    }
    else if (this.filterType=="tjedno"){
      this.subTitle = "Tjedni događaji";

    }
  }

  ionViewWillEnter(){
    this.getDogadjaji(this.filterType);
  }

  getDogadjaji(action:string){
    this.izvidnikProvider.getDogadjaji(action).then(res=>{
      if (res != null && res.stavke != null && res.stavke.length > 0)
      this.dogadjaji = res.stavke
    })

    }
    
    Unos(){

      this.navCtrl.push('UnosIzvidaPage')

    }

    otvoriDetalje(dogadjaj){
      // this.izvidnikProvider.getDetalji('detalji', dogadjaj).then(res=>{
      //   console.log(dogadjaj)
      console.log(dogadjaj);
       this.global.modal = this.modalCtrl.create('DetaljiPage', { data: dogadjaj },{ enableBackdropDismiss: false });
       this.global.modal.present();
 
       this.global.modal.onDidDismiss( data =>{

          this.dogadjaji.splice(0, this.dogadjaji.length)
          this.getDogadjaji(this.filterType);

          this.global.modal = null;
        })
    // })    
  }

  getBackgroundImage(item) {
    
    if (item.slike != null && item.slike.length > 0)
    {
      //return "";
      return " url('data:image/png;base64," + item.slike[0].base64image + "')";
      //return " url('data:image/png;base64,')";
    }
        
    else
        return "url('assets/images/gauge.svg')";
}

}
