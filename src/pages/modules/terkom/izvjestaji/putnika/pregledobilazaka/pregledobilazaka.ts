...
@IonicPage()
@Component({
	selector: 'page-izvjestaji-pregledobilazaka',
	templateUrl: 'pregledobilazaka.html'
})
export class TerkomIzvjestajiPutnikaPregledObilazakaPage extends BasePage {

	datumOd: string = new Date().toISOString();
	datumDo: string = new Date().toISOString();
	public obilasci = new Array<any>();
	periodDana: number = 0;
	...
	prikazi() {
		...
		loading.present().then(() => {
			this.izvjestajiService.getPregledObilazaka(params)
			.then(data => {			
