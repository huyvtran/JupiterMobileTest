import {Component} from '@angular/core';
import {NavController, IonicPage, NavParams} from 'ionic-angular';

import {BasePage} from '../../../../../providers/base/base-page';
import * as ICore from '../../../../../interfaces/iCore';

@IonicPage()
@Component({selector: 'page-hrm-imenik-det', templateUrl: 'imenik-det.html'})
export class HrmImenikDetPage extends BasePage {
    item : any = {};
    img : any;
    defaultDate : String = new Date(new Date().setFullYear(1700, 0, 1)).toISOString();
    defaultImage : any = "https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcTqXSYBLBOU_P1Q5H62ae3uxVzO" +
            "8O3eErJH5DgbBBMP6mYD0crl5Q";

    //za header accordiona
    opci : string = "Pozicija";
    placa : string = "Organizacijski podaci";
    radnomjesto : string = "Kontakt";

    hrKadroviId: number;

    constructor(public navCtrl : NavController, public navParams : NavParams) {
        super();

        console.log(this.navParams);
        console.log(this.navParams.data);
        this.hrKadroviId = this.navParams.get('hrkadroviid');
        console.log("hrkadroviid");
        console.log(this.hrKadroviId );

        this
            .navParams
            .get('godina');

        this
            .getData()
            .then(x => this.item = x);

        this
            .getImgData()
            .then(x => {
                this.img = x;
            });
    }

    getData() {
        let dataDef : ICore.IData = {
            "queries": [
                {
                    "query": "spMobHrmProfil",
                    "params": {
                        "action": "detalji",
                        "hrKadroviId": this.hrKadroviId
                    },
                    "singlerow": true
                }
            ]
        }
        return this
            .global
            .getData(dataDef, true);

    }

    getImgData() {
        let dataDef : ICore.IData = {
            "queries": [
                {
                    "query": "spMobHrmProfil",
                    "params": {
                        "action": "slika",
                        "hrkadroviid": this.hrKadroviId
                    },
                    "singlerow": true
                }
            ]
        }
        return this
            .global
            .getData(dataDef, true);

    }


    getBackgroundImage(): string {
        if (this.img != null)
            return "url('data:image/png;base64," + this.img.slika;
        else {
            return "url(data:image/png;base64,);";
        }
            
            //return "url('../assets/images/user-unknown.png')";
    }


    

}
