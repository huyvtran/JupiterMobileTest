import {Component} from '@angular/core';
import {NavController, IonicPage} from 'ionic-angular';

import {BasePage} from '../../../../../providers/base/base-page';
import * as ICore from '../../../../../interfaces/iCore';

@IonicPage()
@Component({selector: 'page-hrm-profil-detalji-djelatnika', templateUrl: 'detalji-djelatnika.html'})
export class HrmProfilDetaljiDjelatnikaPage extends BasePage {
    item : any = {};
    img : any;
    defaultDate : String = new Date(new Date().setFullYear(1700, 0, 1)).toISOString();
    defaultImage : any = "https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcTqXSYBLBOU_P1Q5H62ae3uxVzO" +
            "8O3eErJH5DgbBBMP6mYD0crl5Q";

    //za header accordiona
    opci : string = "Opći podaci";
    placa : string = "Podaci za obradu plaće";
    radnomjesto : string = "Radno mjesto";
    constructor(public navCtrl : NavController) {
        super();

        this
            .getData()
            .then(x => this.item = x);

        this
            .getImgData()
            .then(x => {
                this.img = x;
            });
    }

    getData() {
        let dataDef : ICore.IData = {
            "queries": [
                {
                    "query": "spMobHrmProfil",
                    "params": {
                        "action": "detalji",
                        "operaterid": "@@operaterid"
                    },
                    "singlerow": true
                }
            ]
        }
        return this
            .global
            .getData(dataDef, true);

    }

    getImgData() {
        let dataDef : ICore.IData = {
            "queries": [
                {
                    "query": "spMobHrmProfil",
                    "params": {
                        "action": "slika",
                        "operaterid": "@@operaterid"
                    },
                    "singlerow": true
                }
            ]
        }
        return this
            .global
            .getData(dataDef, true);

    }


    getBackgroundImage(): string {
        if (this.img != null)
            return "url('data:image/png;base64," + this.img.slika;
        else
            return " url('data:image/png;base64,')";
            //return "url('../assets/images/user-unknown.png')";
    }

}
